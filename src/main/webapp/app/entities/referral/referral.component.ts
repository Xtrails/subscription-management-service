...
export default defineComponent({
  ...
  setup() {
    ...
    const referrals: Ref<IReferral[]> = ref([]);
    ...
    const retrieveReferrals = async () => {
      ...
      try {
        ...
        const res = await referralService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        referrals.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    const prepareRemove = (instance: IReferral) => {
    ...
    const removeReferral = async () => {
      try {
        await referralService().delete(removeId.value);
        const message = t$('subscriptionManagementServiceApp.referral.deleted', { param: removeId.value }).toString();
    ...
    watch([referrals, page], async ([data, page], [_prevData, prevPage]) => {
