        ...
        <div>
          ...
          <div class="form-group">
            <label
              ...
              v-text="t$('subscriptionManagementServiceApp.payment.paymentDttm')"
              for="payment-paymentDttm"
            ></label>
            <b-input-group class="mb-3">
              <b-input-group-prepend>
                <b-form-datepicker
                  aria-controls="payment-paymentDttm"
                  v-model="v$.paymentDttm.$model"
                  name="paymentDttm"
                  class="form-control"
              ...
              <b-form-input
                id="payment-paymentDttm"
                data-cy="paymentDttm"
                type="text"
                class="form-control"
                name="paymentDttm"
                :class="{ valid: !v$.paymentDttm.$invalid, invalid: v$.paymentDttm.$invalid }"
                v-model="v$.paymentDttm.$model"
                required
              />
            </b-input-group>
            <div v-if="v$.paymentDttm.$anyDirty && v$.paymentDttm.$invalid">
              <small class="form-text text-danger" v-for="error of v$.paymentDttm.$errors" :key="error.$uid">{{ error.$message }}</small>
            </div>
          </div>
          <div class="form-group">
            <label class="form-control-label" v-text="t$('subscriptionManagementServiceApp.payment.hashSum')" for="payment-hashSum"></label>
            <input
              type="text"
              class="form-control"
              name="hashSum"
              id="payment-hashSum"
              data-cy="hashSum"
              :class="{ valid: !v$.hashSum.$invalid, invalid: v$.hashSum.$invalid }"
              v-model="v$.hashSum.$model"
              required
            />
            <div v-if="v$.hashSum.$anyDirty && v$.hashSum.$invalid">
              <small class="form-text text-danger" v-for="error of v$.hashSum.$errors" :key="error.$uid">{{ error.$message }}</small>
            </div>
          </div>
          <div class="form-group">
