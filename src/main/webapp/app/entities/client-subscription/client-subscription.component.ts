...
export default defineComponent({
  ...
  setup() {
    ...
    const retrieveClientSubscriptions = async () => {
      ...
      try {
        ...
        const res = await clientSubscriptionService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        clientSubscriptions.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    const removeClientSubscription = async () => {
      try {
        await clientSubscriptionService().delete(removeId.value);
        const message = t$('subscriptionManagementServiceApp.clientSubscription.deleted', { param: removeId.value }).toString();
    ...
    watch([clientSubscriptions, page], async ([data, page], [_prevData, prevPage]) => {
