      ...
      <table class="table table-striped" aria-describedby="clientSubscriptions">
        <thead>
          <tr>
            ...
            <th scope="row" @click="changeOrder('startDttm')">
              <span v-text="t$('subscriptionManagementServiceApp.clientSubscription.startDttm')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'startDttm'"></jhi-sort-indicator>
            </th>
            <th scope="row" @click="changeOrder('endDttm')">
              <span v-text="t$('subscriptionManagementServiceApp.clientSubscription.endDttm')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'endDttm'"></jhi-sort-indicator>
            </th>
            ...
            <th scope="row" @click="changeOrder('subscriptionDetails.id')">
              <span v-text="t$('subscriptionManagementServiceApp.clientSubscription.subscriptionDetails')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'subscriptionDetails.id'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="clientSubscription in clientSubscriptions" :key="clientSubscription.id" data-cy="entityTable">
            ...
            <td>{{ clientSubscription.startDttm }}</td>
            <td>{{ clientSubscription.endDttm }}</td>
            ...
            <td>
              <div v-if="clientSubscription.subscriptionDetails">
                <router-link
                  :to="{ name: 'SubscriptionDetailsView', params: { subscriptionDetailsId: clientSubscription.subscriptionDetails.id } }"
                  >{{ clientSubscription.subscriptionDetails.id }}</router-link
                >
              </div>
            </td>
            <td class="text-right">
